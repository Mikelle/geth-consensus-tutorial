...
func (s *PayloadStore) SavePayload(ctx context.Context, p *Payload) error {
	query := `
		INSERT INTO payloads (block_number, block_hash, parent_hash, payload_data, timestamp)
		VALUES ($1, $2, $3, $4, $5)
	...
	`
	_, err := s.pool.Exec(ctx, query, p.BlockNumber, p.BlockHash, p.ParentHash, p.PayloadData, p.Timestamp)
	return err
}
...
func (s *PayloadStore) GetPayloadByNumber(ctx context.Context, number uint64) (*Payload, error) {
	query := `SELECT block_number, block_hash, parent_hash, payload_data, timestamp FROM payloads WHERE block_number = $1`
	row := s.pool.QueryRow(ctx, query, number)

	var p Payload
	if err := row.Scan(&p.BlockNumber, &p.BlockHash, &p.ParentHash, &p.PayloadData, &p.Timestamp); err != nil {
		return nil, err
	}
	return &p, nil
}
...
func (s *PayloadStore) GetPayloadByHash(ctx context.Context, hash string) (*Payload, error) {
	query := `SELECT block_number, block_hash, parent_hash, payload_data, timestamp FROM payloads WHERE block_hash = $1`
...
func (s *PayloadStore) GetLatestPayload(ctx context.Context) (*Payload, error) {
	query := `SELECT block_number, block_hash, parent_hash, payload_data, timestamp FROM payloads ORDER BY block_number DESC LIMIT 1`
...
func (s *PayloadStore) GetPayloadsAfter(ctx context.Context, afterNumber uint64, limit int) ([]*Payload, error) {
	...
	for rows.Next() {
		var p Payload
		if err := rows.Scan(&p.BlockNumber, &p.BlockHash, &p.ParentHash, &p.PayloadData, &p.Timestamp); err != nil {
			return nil, err
		}
